...
// Three depth layers give natural parallax when the camera moves on scroll.
...
export function ThreeBackground() {
  ...
  useEffect(() => {
    ...
    // ── Ground pushed further down (partially cut off to keep it low-lying)
    ...
    layers.forEach(({ z, cols, hScale, gapRatio, fill, fOp, stroke, sOp }) => {
    ...
    const starMat = new THREE.PointsMaterial({
      color: isDark ? 0x888888 : 0x888888,
      size: 0.16,
      map: starTex,
      transparent: true,
    ...
    let targetCamY = 0;
    const onScroll = () => {
      // Camera drifts UP as page scrolls down → buildings lag behind text → depth illusion
      targetCamY = +(window.scrollY / (window.innerHeight || 1)) * 0.7;
    };
    window.addEventListener('scroll', onScroll, { passive: true });

    // ── Render loop (parallax only — city is static) ──────────────────────
    let raf: number;
    const tick = () => {
      if (disposed) return;
    ...
    const onResize = () => {
      ...
      renderer.setSize(parent.offsetWidth, parent.offsetHeight, false);
    };
